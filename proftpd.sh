...
# Backup proftpd.conf 
cp /etc/proftpd/proftpd.conf /etc/proftpd.conf.original

# set options in proftpd.conf and override the standard template
...
#Adding nologin shell in shells file if not existing
ShellsFile="/etc/shells"

 if grep -q /usr/sbin/nologin "$ShellsFile"; then 	# looking for an existing string with grep
   echo "Entry already existing" >>/dev/null 		# output text and redirect the output to teh data grave
 else
 echo "/usr/sbin/nologin" >> /etc/shells			#append the string to the file if it's not existing.			
 fi
...
sudo service proftpd restart 					# restart service
exit
